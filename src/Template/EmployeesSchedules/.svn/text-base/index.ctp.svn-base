...
<section class="content-header">
  <h1>
    <?php echo !empty($departmentName) ? 'Employees Schedules - '.$departmentName : 'Employees Schedules'; ?>
  </h1>
  <ol class="breadcrumb">
        	...
        	<div class="tab-pane active" id="settings">
                <div class="box-header">
                     ...
                     <?php if($roleId == 3 || $roleId == 5) { ?>
                        <?php echo $this->Html->link(__('<i class="fa fa-fw fa-file"></i> Upload CSV'),['controller' => 'EmployeesSchedules', 'action' => 'import-csv'],['class' => 'btn btn-primary ajax-modal-popup', 'escape' => false]); ?>
                     	<?php echo $this->Html->link(__('Sample CSV'),['controller' => 'EmployeesSchedules', 'action' => 'download-csv'],['class' => 'btn btn-primary', 'escape' => false]); ?>
                     <?php }?>
                     </div>
                 
                  <?php 
                    if(!empty($search) || !empty($department))
                      echo $this->Html->link(__('<i class="fa fa-fw fa-refresh"></i> Reset'),['controller' => 'EmployeesSchedules', 'action' => 'index'],['class' => 'btn btn-default', 'escape' => false]); 
                  ?>
                              ...
                              <?php foreach ($employeesSchedules as $employee): ?>
                                <tr role="row">
                                    ...
                                    <table>
                                        ...
                                        <?php if(!empty($employee->schedule)) {?>
                                        <td>
                                        	<div class="input text form-control"><label>Service</label></div>
                                        	<div class="input text form-control"><label>Time</label></div>
                                       		<div class="input text form-control"><label>Date</label></div>
                                       	</td>
                                       		<?php echo $this->Form->create(null, [
                                       		        'id'=>'edit-schedule-'.$j,
                                                    'url' => ['controller' => 'EmployeesSchedules', 'action' => 'edit', $employee->id]
                                                ]); ?>
                                            <?php 
                                            $i = 0;
                                            $schedules = json_decode($employee->schedule, true);
                                            foreach ($schedules as $sched_key => $sched_val):
                                                ...
                                                ?>
                                                	...
                                                	<td>
                                                	    <?php 
                                                            echo $this->Form->input('service_team[]',[
                                                                'type' => 'text',
                                                                'class' => 'form-control required',
                                                                'label' => false,
                                                                'value' => $s_val['service_team']
                                                            ]);
                                                        ?>
                                    ...
                                    <td class="actions">
                                    	...
                                    	<?php echo $this->Html->link(__('<i class="fa fa-fw fa-eye"></i>'),['controller' => 'EmployeesSchedules', 'action' => 'view', $employee->id],['class' => 'btn btn-xs btn-info', 'escape' => false, 'title' => 'View']); ?></td>
                                    </td>
                              </tr>
                              <?php $j++; endforeach; ?>
...
<script>
    ...
    function resiz_schedule() {
    	var element_width = $('.content').width() - ($('.th_photo').width() + $('.th_detail').width() + $('.th_action').width()) - 100;
